...
JSPG['SceneHandler'] = new (function () {
    ...
    this.showScene = function (name, sceneConfig)  {
        ...
        JSPG.ActionHandler.hideActionButtons();
        ...
        // Prepare Actions
        JSPG.ActionHandler.setSceneActions(scene.actions);
        ...
        if (framesAmount == 0) {
            ...
            JSPG.ActionHandler.showActionButtons()
...
JSPG['ActionHandler'] = new (function () {
    ...
    this.setSceneActions = function (actions) {
        ...
        for (let idx = 0; idx < actions.length; ++idx) {
            ...
            $(this.SELECTORS.CONTAINER).append(this.HTML.BUTTON.format('uid', action.id))
            const $btn = $(this.SELECTORS.BUTTON_BY_ID.format('uid', action.id))

            const html = action.icon == null
                         ? action.name
                         : `${action.icon.get()}${action.name}`
            $btn.html(html)
            $btn.css("opacity", 0)
            $btn.off()
